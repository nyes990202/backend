...
<div class="container">
<form method="POST" enctype="multipart/form-data" action='/admin/news/update/{{$news->id}}'>
    @csrf
    <div class="form-group">
        <label for="title">標題 <small class="text-danger">(限制字數最多20)</small></label>
        <input name="title" type="text" class="form-control" id="title" required value="{{$news->title}}">
    </div>
    <div class="form-group">
        <label for="sub_title">副標題</label>
        <input name="sub_title" type="text" class="form-control" id="sub_title" required value="{{$news->id}}">

      </div>

      <div class="form-group">
        <div for="img_url">原本主要圖片<small class="text-danger">(建議上傳寬高比例4:3)</small></div>
        <img  class="figure" src="{{$news->img_url}}" alt="">
      </div>
      ...
      <div class="form-group">
        ...
        <textarea name="text" class="form-control" id="text" rows="3" required>{{$news->text}}</textarea>
      </div>
